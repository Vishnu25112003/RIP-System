...
const UserVerificationPage: React.FC = () => {
  ...
  const fetchPendingUsers = async () => {
    try {
      const res = await fetch(
        "http://localhost:5000/api/userverification/pending-users"
      );
  ...
  const updateUserStatus = async (
    ...
    try {
      const res = await fetch(
        "http://localhost:5000/api/userverification/update-status",
        {
          method: "PUT",
          ...
          <tbody>
            {users.length > 0 ? (
              users.map((user, i) => (
                <tr
                  key={user._id}
                  className={`hover:bg-neonpurple transition ${
                    i === users.length - 1 ? "rounded-b-xl" : ""
                  }`}
                >
